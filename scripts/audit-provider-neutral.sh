...
section() { printf '\n=== %s ===\n' "$1"; }
status_line() { local level="$1"; local message="$2"; printf '%-4s %s\n' "$level" "$message"; }

assert_no_match() {
  local severity="$1"; local title="$2"; shift 2
  local out rc; out=$("$@" 2>&1); rc=$?
  if [[ "$rc" -eq 0 ]]; then
    printf '%s\n' "$out"
    if [[ "$severity" == "FAIL" ]]; then FAIL_COUNT=$((FAIL_COUNT + 1)); status_line "FAIL" "$title"; else WARN_COUNT=$((WARN_COUNT + 1)); status_line "WARN" "$title"; fi
    return 1
  fi
  if [[ "$rc" -gt 1 ]]; then
    printf '%s\n' "$out"; printf 'command failed with exit code %d\n' "$rc"
    FAIL_COUNT=$((FAIL_COUNT + 1)); status_line "FAIL" "$title (audit command failed)"; return 1
  fi
  status_line "PASS" "$title"; return 0
}

assert_match() {
  local severity="$1"; local title="$2"; shift 2
  local out rc; out=$("$@" 2>&1); rc=$?
  if [[ "$rc" -eq 0 ]]; then printf '%s\n' "$out"; status_line "PASS" "$title"; return 0; fi
  if [[ "$rc" -gt 1 ]]; then printf '%s\n' "$out"; printf 'command failed with exit code %d\n' "$rc"; fi
  if [[ "$severity" == "FAIL" ]]; then FAIL_COUNT=$((FAIL_COUNT + 1)); status_line "FAIL" "$title"; else WARN_COUNT=$((WARN_COUNT + 1)); status_line "WARN" "$title"; fi
  return 1
}

require_command() { local command_name="$1"; if ! command -v "$command_name" >/dev/null 2>&1; then printf 'ERROR: %s is required\n' "$command_name" >&2; exit 2; fi; }
check_direct_dependency_absent() { local manifest="$1"; rg -n -S '^agent-client-protocol[[:space:]]*=' "$manifest" >/dev/null 2>&1; }
check_direct_dependency_used() { local source_dir="$1"; rg -n -S --glob '*.rs' 'agent[_-]client[_-]protocol' "$source_dir" >/dev/null 2>&1; }
...
if [[ ! -f crates/llm-provider/Cargo.toml || ! -f crates/tools-provider/Cargo.toml ]]; then
  status_line "FAIL" "provider Cargo.toml files are present"; FAIL_COUNT=$((FAIL_COUNT + 1))
else
  if check_direct_dependency_absent "crates/llm-provider/Cargo.toml"; then status_line "FAIL" "llm-provider has no direct agent-client-protocol dependency"; FAIL_COUNT=$((FAIL_COUNT + 1)); else status_line "PASS" "llm-provider has no direct agent-client-protocol dependency"; fi
  if check_direct_dependency_used "$TOOLS"; then status_line "PASS" "tools-provider ACP dependency is justified by source usage outside visual UX"; else
    if rg -n -S '^agent-client-protocol[[:space:]]*=' crates/tools-provider/Cargo.toml >/dev/null 2>&1; then status_line "FAIL" "tools-provider declares ACP without source usage"; FAIL_COUNT=$((FAIL_COUNT + 1)); else status_line "PASS" "tools-provider does not declare an unnecessary ACP dependency"; fi
  fi
fi

if command -v cargo >/dev/null 2>&1; then
  llm_tree=$(cargo tree -p llm-provider --depth 1 2>/dev/null || true)
  if printf '%s\n' "$llm_tree" | rg -n 'agent-client-protocol' >/dev/null 2>&1; then printf '%s\n' "$llm_tree" | rg -n 'agent-client-protocol'; status_line "FAIL" "llm-provider dependency graph contains agent-client-protocol"; FAIL_COUNT=$((FAIL_COUNT + 1)); else status_line "PASS" "llm-provider dependency graph excludes agent-client-protocol"; fi
else
  status_line "WARN" "cargo unavailable; dependency graph checks skipped"; WARN_COUNT=$((WARN_COUNT + 1))
fi
...
if [[ "$FAIL_COUNT" -eq 0 ]]; then
  if [[ "$WARN_COUNT" -eq 0 ]]; then echo "PASS: provider-neutral architecture audit is clean."; else echo "PASS: no hard-boundary failures; warnings are informational and non-blocking."; fi
  exit 0
fi
