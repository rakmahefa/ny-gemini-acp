...
impl TurnIntegrity {
    ...
    pub(super) fn thinking_started(&mut self)->Result<(),IntegrityError>{self.ensure_active("thinking_started")?;if self.assistant!=StreamPhase::Active{return Err(IntegrityError::new("thinking requires an active assistant stream"))}if self.thinking==StreamPhase::Active{return Err(IntegrityError::new("thinking stream is already active"))}self.thinking=StreamPhase::Active;Ok(())}
    pub(super) fn thinking_delta(&self)->Result<(),IntegrityError>{self.ensure_active("thinking_delta")?;if self.thinking!=StreamPhase::Active{return Err(IntegrityError::new("thinking_delta requires an active thinking stream"))}Ok(())}
    pub(super) fn thinking_completed(&mut self)->Result<(),IntegrityError>{self.ensure_active("thinking_completed")?;if self.thinking!=StreamPhase::Active{return Err(IntegrityError::new("thinking_completed requires an active thinking stream"))}self.thinking=StreamPhase::Idle;Ok(())}
