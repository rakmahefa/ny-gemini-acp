...
impl Store {
    pub async fn begin_turn(&self, id: &str) -> Result<(Session, watch::Receiver<bool>, u64), TurnError> {
        ...
        live.insert(id.to_string(), Live { session: session.clone(), cancel: cancellation, busy: true, generation: 1 });
        Ok((session, rx, 1))
    }
    ...
    pub async fn cancel_all(&self) {
        let live = self.live.read().await;
        for (id, entry) in live.iter() { entry.cancel.cancel(); tracing::debug!(session = %id, "session cancellation requested"); }
    }
    ...
    pub async fn fork(&self, source_id: &str) -> Result<Session> {
        ...
        self.live.write().await.insert(forked.id.clone(), Live { session: forked.clone(), cancel: Cancellation::new(), busy: false, generation: 0 });
        Ok(forked)
    }
}
