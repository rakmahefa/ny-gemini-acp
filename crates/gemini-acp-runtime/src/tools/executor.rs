...
impl<'a> ToolExecutor<'a> {
    ...
    pub async fn execute(&self, tool_name: &str, arguments: &Value) -> ToolResult {
        ...
        let outcome = if tool_name == "shell_exec" {
            match self
                .execute_shell_via_acp_terminal(arguments, &call_id)
                .await
            {
                Ok(outcome) => outcome,
                Err(error) => {
    ...
    fn emit_tool_call(
        ...
        let tool = AcpToolCall::new(call_id.clone(), info.title.clone())
            ...
            .meta(lifecycle_meta(&info.title, lifecycle, None, None));
    ...
    fn emit_lifecycle(&self, call_id: &ToolCallId, lifecycle: &ToolLifecycle, tool_name: &str) {
        self.emit_update(
            ...
            vec![],
            vec![],
            Some(lifecycle_meta(tool_name, lifecycle, None, None)),
        );
    }
...
fn terminal_lifecycle_meta(
    ...
    if let Some(output) = output {
        ...
        meta.insert(
            ...
            json!({ "terminal_id": terminal_id, "data": preview }),
        );
    }
