...
impl<'a> ToolExecutor<'a> {
    ...
    pub async fn execute(&self, tool_name: &str, arguments: &Value) -> ToolResult {
        ...
        if session_cancelled(self.session_id.0.as_ref()) {
            return self.finish_terminal(TerminalFinish { call_id: &call_id, lifecycle: &mut lifecycle, tool_name, arguments, content: "outil annulé avant son démarrage".into(), is_ok: false, cancelled: true, reason: Some("cancelled"), terminal_id: None, terminal_meta: None });
        }
        ...
        if needs_permission {
            ...
            match self.request_permission(&request, &call_id).await {
                PermissionResult::Allow => {
                    if session_cancelled(self.session_id.0.as_ref()) {
                        return self.finish_terminal(TerminalFinish { call_id: &call_id, lifecycle: &mut lifecycle, tool_name, arguments, content: format!("{} ({}) annulé avant le démarrage de l'exécution.", request.kind.label(), request.summary), is_ok: false, cancelled: true, reason: Some("cancelled"), terminal_id: None, terminal_meta: None });
                    }
                    lifecycle.transition(ToolLifecycleState::Executing).expect("permission -> executing must be legal");
                    self.emit_lifecycle(&call_id, &lifecycle, tool_name);
                }
                PermissionResult::Reject => {
                    return self.finish_terminal(TerminalFinish { call_id: &call_id, lifecycle: &mut lifecycle, tool_name, arguments, content: format!("{} ({}) refusé par l'utilisateur.", request.kind.label(), request.summary), is_ok: false, cancelled: false, reason: Some("user-rejected"), terminal_id: None, terminal_meta: None });
                }
                PermissionResult::Cancelled => {
                    return self.finish_terminal(TerminalFinish { call_id: &call_id, lifecycle: &mut lifecycle, tool_name, arguments, content: format!("{} ({}) annulé pendant la demande d'autorisation.", request.kind.label(), request.summary), is_ok: false, cancelled: true, reason: Some("cancelled"), terminal_id: None, terminal_meta: None });
                }
                PermissionResult::TransportError(error) => {
                    return self.finish_terminal(TerminalFinish { call_id: &call_id, lifecycle: &mut lifecycle, tool_name, arguments, content: format!("Échec de la demande de permission ACP : {error}"), is_ok: false, cancelled: false, reason: Some("permission-error"), terminal_id: None, terminal_meta: None });
                }
            }
        } else {
            if session_cancelled(self.session_id.0.as_ref()) {
                return self.finish_terminal(TerminalFinish { call_id: &call_id, lifecycle: &mut lifecycle, tool_name, arguments, content: "outil annulé avant son exécution".into(), is_ok: false, cancelled: true, reason: Some("cancelled"), terminal_id: None, terminal_meta: None });
            }
        ...
        } else { self.execute_registry(tool_name, arguments).await };
