...
pub fn unbind_session_cancellation(session_id: &str) {
    cancellation_map().lock().expect("session cancellation mutex poisoned").remove(session_id);
}

pub fn session_cancelled(session_id: &str) -> bool {
    cancellation_map().lock().expect("session cancellation mutex poisoned").get(session_id).is_some_and(Cancellation::is_cancelled)
}
