...
use super::types::{
    CardBodyKind, ResultUpdate, MAX_DIFF_OLD_TEXT_BYTES, MAX_RESULT_LOCATIONS,
...
pub fn result_update(
    ...
    match tool_name {
        "file_read" => {
            ...
            ResultUpdate {
                ...
                content: vec![ux_card(
                    ...
                    phase,
                    args,
                    Some((&body, CardBodyKind::Output, !is_ok)),
                    terminal_id,
                )],
        ...
        "AskUserQuestion" => {
            ...
            ResultUpdate {
                ...
                content: vec![ux_card(
                    ...
                    phase,
                    args,
                    Some((&body, CardBodyKind::Content, !is_ok)),
                    terminal_id,
                )],
                locations: vec![],
            }
        ...
        "FollowUp" => {
            let body = if is_ok {
                render_follow_up_result(result)
            } else {
                result.to_owned()
            };
            ResultUpdate {
...
pub fn classify_risk(name: &str, args: &Value) -> RiskLevel {
    match name {
        // Single classification entry point: `ShellSandbox::classify` runs the
        // full validation and falls back to `Critical` when the command would
        // be refused, so permission policies always see the worst case.
        "shell_exec" => arg_str(args, "command")
            .map(|command| ShellSandbox::new().classify(command))
...
fn render_ask_user_result(result: &str) -> String {
    ...
    if answers.is_empty() {
        return "Aucune réponse sélectionnée.".into();
    }
    answers
        .iter()
...
fn file_location(args: &Value, cwd: &Path) -> Vec<Value> {
    arg_str(args, "path")
        .map(|path| vec![location(&resolve_path(path, cwd), None)])
        .unwrap_or_default()
}
...
fn filesystem_result_locations(tool_name: &str, result: &str, cwd: &Path) -> Vec<Value> {
    ...
    result
        .lines()
        .take(MAX_RESULT_LOCATIONS)
        .filter_map(|line| {
            let path = line.trim();
            if path.is_empty() {
                None
            } else {
                Some(location(&resolve_path(path, cwd), None))
            }
        })
        .collect()
}
...
fn search_result_locations(result: &str, cwd: &Path) -> Vec<Value> {
    ...
    for line in result.lines() {
        ...
        if seen.insert(key) {
            locations.push(location(&resolved, Some(line_number)));
        }
...
mod tests {
    ...
    fn risk_for_file_write_is_medium() {
        assert_eq!(
            classify_risk("file_write", &serde_json::json!({})),
            RiskLevel::Medium
        );
    }
}
