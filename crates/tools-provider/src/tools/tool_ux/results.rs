...
pub fn result_update(
    ...
    match tool_name {
        ...
        "search" | "search_and_read" => {
            ...
            ResultUpdate {
                status,
                content: vec![ux_card(tool_name, phase, args, Some((&rendered, CardBodyKind::Output, !is_ok)), terminal_id)],
...
pub(crate) fn arg_str<'a>(args: &'a Value, key: &str) -> Option<&'a str> { args.get(key).and_then(Value::as_str) }
