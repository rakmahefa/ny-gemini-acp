...
impl ToolInfo {
    pub fn build(name: &str, args: &Value, cwd: &Path, terminal_id: Option<&str>) -> Self {
        ...
        info.kind = ui_kind(name);
        info
    }
}

fn ui_kind(name: &str) -> agent_runtime::ToolUiKind {
    match name {
        "file_read" => agent_runtime::ToolUiKind::FileRead,
        "file_write" => agent_runtime::ToolUiKind::FileWrite,
        "file_edit" => agent_runtime::ToolUiKind::FileEdit,
        "glob" => agent_runtime::ToolUiKind::Glob,
        "list_directory" => agent_runtime::ToolUiKind::DirectoryList,
        "search" => agent_runtime::ToolUiKind::Search,
        "search_and_read" => agent_runtime::ToolUiKind::SearchAndRead,
        "shell_exec" => agent_runtime::ToolUiKind::Shell,
        "replace_in_file" => agent_runtime::ToolUiKind::ReplaceInFile,
        "AskUserQuestion" => agent_runtime::ToolUiKind::AskUserQuestion,
        _ => agent_runtime::ToolUiKind::Generic,
    }
}
...
fn file_read(args: &Value, cwd: &Path) -> ToolInfo {
    ...
    ToolInfo {
        kind: agent_runtime::ToolUiKind::Generic,
        ...
        locations: vec![json!({ "path": resolve_path(path, cwd), "line": offset })],
    }
}
...
fn file_write(args: &Value, cwd: &Path) -> ToolInfo {
    ...
    let diff = json!({ "type": "diff", "path": resolved, "oldText": read_old_text(&resolved), "newText": content });
    let input = format!("{}  ·  {} chars", display_path(path, cwd), content.chars().count());
    ToolInfo {
        kind: agent_runtime::ToolUiKind::Generic,
        title: format!("Write {}", display_path(path, cwd)),
        content: vec![ux_card("file_write", "⏳ pending", args, Some((&input, CardBodyKind::Input, false)), None), diff],
        locations: vec![json!({ "path": resolved })],
    }
}
...
fn file_edit(args: &Value, cwd: &Path) -> ToolInfo {
    ...
    let diff = json!({ "type": "diff", "path": resolved, "oldText": old_text, "newText": new });
    ...
    ToolInfo {
        ...
        content: vec![ux_card("file_edit", "⏳ pending", args, Some((&input, CardBodyKind::Input, false)), None), diff],
        locations: vec![json!({ "path": resolved })],
    }
}
...
fn glob(args: &Value, cwd: &Path) -> ToolInfo {
    ...
    ToolInfo {
        ...
        locations: vec![json!({ "path": resolve_path(path, cwd) })],
    }
}
...
fn shell_exec(args: &Value, terminal_id: Option<&str>) -> ToolInfo {
    ...
    if let Some(id) = terminal_id { content.push(json!({ "type": "terminal", "id": id })); }
    ToolInfo { kind: agent_runtime::ToolUiKind::Generic, title: if command.is_empty() { "Terminal".into() } else { truncate(command, 96) }, content, locations: vec![] }
}

fn ask_user_question(args: &Value) -> ToolInfo {
    let title = ask_user_title(args);
    let body = render_ask_user_input(args);
    ToolInfo { kind: agent_runtime::ToolUiKind::Generic, title, content: vec![ux_card("AskUserQuestion", "⏳ waiting for user", args, Some((&body, CardBodyKind::Content, false)), None)], locations: vec![] }
}
...
fn follow_up(args: &Value) -> ToolInfo {
    ...
    ToolInfo { kind: agent_runtime::ToolUiKind::Generic, title: format!("Follow-up · {}", truncate(label, 72)), content: vec![ux_card("FollowUp", "⏳ pending", args, Some((&input, CardBodyKind::Content, false)), None)], locations: vec![] }
}
...
fn generic(name: &str, args: &Value) -> ToolInfo {
    ...
    ToolInfo { kind: agent_runtime::ToolUiKind::Generic, title: name.to_owned(), content: vec![ux_card(name, "⏳ pending", args, Some((&body, CardBodyKind::Input, false)), None)], locations: vec![] }
}
