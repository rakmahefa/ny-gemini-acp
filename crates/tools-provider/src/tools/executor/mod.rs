...
impl<'a> ToolExecutor<'a> {
    ...
    fn semantic_ui(&self, tool_name: &str, arguments: &Value, info: &ToolInfo) -> ToolUiModel {
        ToolUiModel::pending(
            super::super::tool_ux::tool_ui_kind(tool_name),
            info.title.clone(),
            info.title.clone(),
    ...
    async fn execute_registry(&self, call_id: &ToolCallId, tool_name: &str, arguments: &Value) -> ExecutionOutcome {
        ...
        ExecutionOutcome { result: ToolResult { content: result.content, is_ok: result.is_ok }, terminal_id: None, terminal_meta: None, cancelled }
    }
}
