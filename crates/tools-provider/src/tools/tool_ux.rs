...
fn file_read(args: &Value, cwd: &Path) -> ToolInfo {
    ...
    ToolInfo {
        ...
        content: vec![ux_card(
            ...
            "⏳ pending",
            args,
            Some((&input, CardBodyKind::Input, false)),
            None,
        )],
...
fn ask_user_question(args: &Value) -> ToolInfo {
    ...
    ToolInfo {
        ...
        content: vec![ux_card(
            ...
            "⏳ waiting for user",
            args,
            Some((&body, CardBodyKind::Content, false)),
            None,
        )],
        locations: vec![],
    }
...
fn follow_up(args: &Value) -> ToolInfo {
    ...
    ToolInfo {
        ...
        content: vec![ux_card(
            ...
            "⏳ pending",
            args,
            Some((&input, CardBodyKind::Content, false)),
            None,
        )],
        locations: vec![],
    }
...
fn generic(name: &str, args: &Value) -> ToolInfo {
    ...
    ToolInfo {
        ...
        content: vec![ux_card(
            ...
            "⏳ pending",
            args,
            Some((&body, CardBodyKind::Input, false)),
            None,
        )],
        locations: vec![],
    }
...
fn render_ask_user_input(args: &Value) -> String {
    ...
    for (index, question) in questions.iter().enumerate() {
        ...
        let prompt = question
            .get("question")
            .and_then(Value::as_str)
            .unwrap_or("Question indisponible.");
    ...
    result
        ...
        .filter_map(|line| {
            ...
            if path.is_empty() {
                ...
                Some(ToolCallLocation::new(resolve_path(path, cwd)))
            }
        })
        .collect()
}
...
mod tests {
    ...
    fn core_tools_keep_one_text_card() {
        ...
        for (name, args) in [
            ...
            ("search", serde_json::json!({"pattern":"foo"})),
            ("shell_exec", serde_json::json!({"command":"cargo test"})),
            (
                "AskUserQuestion",
                serde_json::json!({"questions":[{"question":"Continue?","options":[{"label":"Yes"}]}]}),
            ),
        ] {
            let info = ToolInfo::build(name, &args, cwd, None);
            assert!(
                matches!(info.content.first(), Some(ToolCallContent::Content(_))),
