...
fn session_id_error(id: &SessionId) -> AcpError { AcpError::invalid_params().data(serde_json::json!({"session_id": id.to_string(), "error": "identifiant de session invalide"})) }
...
fn build_available_modes() -> Vec<SessionMode> { AcpSessionMode::all().iter().map(|mode| SessionMode::new(session_mode_id(*mode), mode.display_name()).description(mode.description())).collect() }
...
struct ReplayTool<'a> { id: &'a str, name: &'a str, arguments: &'a serde_json::Value, result_text: Option<&'a str>, result_ok: Option<bool>, cwd: &'a std::path::Path }
fn replay_tool_result(cx: &ConnectionTo<Client>, session_id: &SessionId, replay: ReplayTool<'_>) -> Result<(), AcpError> {
    let call_id = ToolCallId::from(replay.id.to_owned());
    let info = ToolInfo::build(replay.name, replay.arguments, replay.cwd, None);
    let is_ok = replay.result_ok.unwrap_or_else(|| replay.result_text.map(|text| !is_rejected_or_cancelled_tool_result(text)).unwrap_or(false));
    cx.send_notification(SessionNotification::new(session_id.clone(), SessionUpdate::ToolCall(ToolCall::new(call_id.clone(), info.title.clone()).kind(info.kind).status(if replay.result_text.is_some() { if is_ok { ToolCallStatus::Completed } else { ToolCallStatus::Failed } } else { ToolCallStatus::InProgress }).content(info.content.clone()).locations(info.locations.clone()).raw_input(bounded_raw_input(replay.arguments))))?
        ;
...
pub async fn handle_new(req: NewSessionRequest, responder: Responder<NewSessionResponse>, state: &AppState) -> Result<(), AcpError> {
    let session = match state.sessions.create(req.cwd.clone(), req.additional_directories.clone(), &state.config.default_model).await {
        Ok(session) => session,
...
pub async fn handle_load(req: LoadSessionRequest, responder: Responder<LoadSessionResponse>, state: &AppState, cx: &ConnectionTo<Client>) -> Result<(), AcpError> {
    ...
    let session = match state.sessions.load(&req.session_id.0, &req.cwd).await {
        Ok(session) => session,
        Err(error) => return responder.respond_with_error(AcpError::invalid_params().data(serde_json::json!({"session_id": req.session_id.to_string(), "error": format!("session introuvable ou workspace incompatible: {error:#}")}))),
    };
    ...
    for entry in &entries { if let HistoryEntry::ToolResult { id, name, content, is_ok } = entry { results_by_id.insert(id.clone(), (name.clone(), content.clone(), *is_ok)); } }
    for (index, entry) in entries.iter().enumerate() {
        match entry {
            HistoryEntry::User { content } => {
                let chunk = ContentChunk::new(ContentBlock::Text(TextContent::new(content.clone()))).message_id(MessageId::from(format!("msg_{index}")));
            ...
            HistoryEntry::ToolCall { id, name, arguments } => {
                ...
                let (result_text, result_ok) = result.map(|(_, content, is_ok)| (Some(content.as_str()), Some(*is_ok))).unwrap_or((None, None));
                replay_tool_result(cx, &req.session_id, ReplayTool { id, name, arguments, result_text, result_ok, cwd: &session.cwd })?;
            }
...
pub async fn handle_delete(req: DeleteSessionRequest, responder: Responder<DeleteSessionResponse>, state: &AppState) -> Result<(), AcpError> {
    if !is_valid_session_id(&req.session_id.0) { return responder.respond_with_error(session_id_error(&req.session_id)); }
    state.turns.cancel_and_wait(&req.session_id.0).await.map_err(|error| AcpError::invalid_params().data(serde_json::json!({"session_id": req.session_id.to_string(), "error": error.to_string()})))?;
    match state.sessions.delete(&req.session_id.0).await {
        Ok(true) => responder.respond(DeleteSessionResponse::new()),
        Ok(false) => responder.respond_with_error(AcpError::invalid_params().data(serde_json::json!({"session_id": req.session_id.to_string(), "error": "session introuvable"}))),
...
pub async fn handle_set_mode(req: SetSessionModeRequest, responder: Responder<SetSessionModeResponse>, state: &AppState, cx: &ConnectionTo<Client>) -> Result<(), AcpError> {
    ...
    let updated = match state.sessions.set_mode(&req.session_id.0, new_mode).await {
        Ok(session) => session,
        Err(error) => return responder.respond_with_error(AcpError::invalid_params().data(serde_json::json!({"session_id": req.session_id.to_string(), "error": format!("impossible de changer le mode: {error:#}")}))),
    };
...
pub async fn handle_fork(req: ForkSessionRequest, responder: Responder<ForkSessionResponse>, state: &AppState) -> Result<(), AcpError> {
    ...
    match state.sessions.fork(&req.session_id.0).await {
        Ok(forked) => responder.respond(ForkSessionResponse::new(SessionId::from(forked.id))),
        Err(error) => responder.respond_with_internal_error(format!("fork de session: {error:#}")),
    }
}
