...
impl TurnGuard {
    ...
    fn session_mut(&mut self) -> &mut gemini_acp_runtime::state::Session { self.session.as_mut().expect("TurnGuard: session déjà consommée") }
...
pub async fn run_turn(store: Arc<Store>, tools: Arc<ToolRegistry>, client: gemini_acp_config::client::Client, req: PromptRequest, responder: Responder<PromptResponse>, cx: ConnectionTo<Client>, semantic: &mut TurnEventEmitter) -> Result<(), AcpError> {
    ...
    if !images.is_empty(){let total=images.len();let upload_call_id=ToolCallId::from(format!("call_{}",uuid::Uuid::new_v4().simple()));safe_session_update(&cx,&session_id,SessionUpdate::ToolCall(ToolCall::new(upload_call_id.clone(),format!("Upload {total} image(s) (Scotty)")).kind(ToolKind::Fetch).status(ToolCallStatus::InProgress)));for(idx,(b64,mime))in images.iter().enumerate(){match client.upload_image(b64,mime).await{Ok(r)=>refs.push(r),Err(e)=>{let content=vec![ToolCallContent::Content(agent_client_protocol::schema::v1::Content::new(ContentBlock::Text(TextContent::new(format!("Upload image {}/{} échoué: {e:#}",idx+1,total))))),];safe_session_update(&cx,&session_id,SessionUpdate::ToolCallUpdate(ToolCallUpdate::new(upload_call_id.clone(),ToolCallUpdateFields::new().status(ToolCallStatus::Failed).content(content))));semantic.turn_failed();span.record("outcome","refusal_upload");return responder.respond(PromptResponse::new(StopReason::Refusal));}}}let content=vec![ToolCallContent::Content(agent_client_protocol::schema::v1::Content::new(ContentBlock::Text(TextContent::new(format!("{total} image(s) uploadée(s) avec succès"))))),];safe_session_update(&cx,&session_id,SessionUpdate::ToolCallUpdate(ToolCallUpdate::new(upload_call_id,ToolCallUpdateFields::new().status(ToolCallStatus::Completed).content(content))));}
    session.messages.push((Role::User,user_text.clone())); let mut total_output=String::new(); let cwd=session.cwd.clone(); let additional_dirs=session.additional_directories.clone(); let registry=&*tools; let mut tool_round=0usize; let mut final_assistant_pushed=false; let mut overflow_retry_count=0usize; let session_mode=session.mode; let mode_getter=||session_mode;
    for round in 0..MAX_TURNS{
        tool_round=round; if *cancel.borrow(){semantic.turn_cancelled();span.record("outcome","cancelled");return responder.respond(PromptResponse::new(StopReason::Cancelled));}
        let history_chars:usize=session.messages.iter().map(|(_,t)|t.len()).sum(); if history_chars>COMPACTION_THRESHOLD_CHARS{compact_messages(&mut session.messages,EMERGENCY_COMPACTION_CHARS);}
        let prompt=build_prompt(session,Some(registry)); let rx=match client.stream(&prompt,&session.model,session.think,&refs).await{Ok(rx)=>rx,Err(e)=>{let note=actionable_error_message(&e);let is_overflow=e.to_string().contains("context")||e.to_string().contains("too long")||e.to_string().contains("tokens");if is_overflow&&overflow_retry_count<1{compact_messages(&mut session.messages,EMERGENCY_COMPACTION_CHARS);overflow_retry_count+=1;continue;}if is_overflow{emit_error_chunk(&cx,&session_id,&message_id,&format!("Context overflow persisted after emergency compaction: {e:#}"));semantic.turn_failed();span.record("outcome","refusal_start");return responder.respond(PromptResponse::new(StopReason::MaxTokens));}emit_error_chunk(&cx,&session_id,&message_id,&note);semantic.turn_failed();span.record("outcome","failed_start");return responder.respond(PromptResponse::new(StopReason::EndTurn));}};
        let is_thinking_model=gemini_acp_config::core::models::resolve(&session.model,gemini_acp_config::core::models::DEFAULT_MODEL).map(|r|gemini_acp_config::core::models::is_thinking_mode(r.mode)).unwrap_or(false);
        let streamed=stream::consume(rx,&mut cancel,&cx,&session_id,&message_id,is_thinking_model,semantic).await?;
        let stream::StreamResult{outcome,assistant,tool_detection_text}=streamed;
        if matches!(outcome,stream::StreamOutcome::Cancelled){semantic.turn_cancelled();span.record("outcome","cancelled");return responder.respond(PromptResponse::new(StopReason::Cancelled));}
        if let stream::StreamOutcome::Failed(e)=&outcome{semantic.turn_failed();span.record("outcome","failed");return responder.respond(PromptResponse::new(map_stop_reason_from_error(e)));}
        ...
        for call in &tool_calls{
            if *cancel.borrow(){semantic.turn_cancelled();return responder.respond(PromptResponse::new(StopReason::Cancelled));}
    ...
    span.record("tool_rounds",tool_round);span.record("chars_output",total_output.chars().count());if !final_assistant_pushed&&!total_output.trim().is_empty(){session.messages.push((Role::Assistant,total_output.clone()));}if let Err(e)=notify_usage(&cx,&session_id,&build_prompt(session,Some(registry)),&total_output){tracing::warn!(session=%session_id,"notify_usage a échoué: {e}");}guard.finish().await;span.record("outcome","end_turn");semantic.turn_completed();responder.respond(PromptResponse::new(StopReason::EndTurn))
}
