...
pub fn capture_gemini_elicitation(
    tool_name: &str,
    input: &serde_json::Value,
) -> Option<GeminiElicitation> {
    ...
    if !matches!(
        normalized.as_str(),
        "askuserquestion" | "askuser" | "elicitation"
    ) {
        return None;
    }
    let questions = extract_ask_user_questions(input)?;
    Some(GeminiElicitation {
...
mod tests {
    ...
    fn captures_gemini_ask_user_question() {
        let input =
            serde_json::json!({"questions":[serde_json::to_value(question(false)).unwrap()]});
    ...
    fn ignores_regular_tool() {
        assert!(
            capture_gemini_elicitation("file_read", &serde_json::json!({"questions":[]})).is_none()
        );
    }
    #[test]
