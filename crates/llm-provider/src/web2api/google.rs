...
pub async fn generate(State(state): State<AppState>, Path(model_path): Path<String>, req: axum::extract::Request) -> Response {
    ...
    let text = match state.client.complete(&prompt, &resolved.name, Some(resolved.think), &refs).await { Ok(text) => text, Err(e) => return json_response(StatusCode::BAD_GATEWAY, serde_json::json!({"error": {"message": format!("upstream error: {e}")}})) };
...
async fn stream_chunks(state: &AppState, prompt: &str, refs: &[String], resolved: &Resolved, model_name: &str) -> Response {
    let mut rx: mpsc::Receiver<StreamResult> = match state.client.stream(prompt, &resolved.name, Some(resolved.think), refs).await { Ok(rx) => rx, Err(e) => return json_response(StatusCode::BAD_GATEWAY, serde_json::json!({"error": {"message": format!("upstream error: {e}")}})) };
        ...
        while let Some(item) = rx.recv().await {
            match item {
                Ok(StreamItem::Text(delta)) => {
                    ...
                    let chunk = serde_json::json!({"candidates": [{"content": {"parts": [{"text": delta}], "role": "model"}, "index": 0}]});
                    if tx.send(Ok(sse_event(chunk))).await.is_err() { return; }
                }
