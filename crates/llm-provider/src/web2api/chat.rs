...
pub async fn handler(State(state): State<AppState>, req: axum::extract::Request) -> Response {
    ...
    if prompt.trim().is_empty() { return json_response(StatusCode::BAD_REQUEST, serde_json::json!({"error": {"message": "empty prompt"}})); }
    let stream = body.get("stream").and_then(Value::as_bool).unwrap_or(false);
    let ctx = Ctx { model_name: model_name.to_string(), cid: format!("chatcmpl-{}", &uuid::Uuid::new_v4().simple().to_string()[..12]), created: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0) };
    if stream && !has_tools { return stream_deltas(&state, &prompt, &resolved, &ctx).await; }
...
async fn stream_deltas(state: &AppState, prompt: &str, resolved: &Resolved, ctx: &Ctx) -> Response {
    let mut rx: mpsc::Receiver<StreamItem> = match state.client.stream(prompt, &resolved.name, Some(resolved.think), &[]).await {
        Ok(rx) => rx,
    ...
    let model_name = ctx.model_name.clone(); let cid = ctx.cid.clone(); let created = ctx.created;
    tokio::spawn(async move {
        let chunk = |delta: Option<&str>, finish: Option<&str>| { let d = match delta { Some(t) => serde_json::json!({"content": t}), None => serde_json::json!({}) }; sse_event(serde_json::json!({"id": cid, "object": "chat.completion.chunk", "created": created, "model": model_name, "choices": [{"index": 0, "delta": d, "finish_reason": finish}]})) };
        let _ = tx.send(Ok(chunk(None, None))).await;
        while let Some(item) = rx.recv().await {
            match item {
                Ok(delta) => { if tx.send(Ok(chunk(Some(delta.as_str()), None))).await.is_err() { return; } }
...
async fn complete(state: &AppState, prompt: &str, resolved: &Resolved, ctx: &Ctx, stream: bool, tools: Option<&[Value]>) -> Response {
    let text: String = match state.client.complete(prompt, &resolved.name, Some(resolved.think), &[]).await {
        Ok(text) => text,
        Err(e) => return json_response(StatusCode::BAD_GATEWAY, serde_json::json!({"error": {"message": format!("upstream error: {e}")}})),
    };
