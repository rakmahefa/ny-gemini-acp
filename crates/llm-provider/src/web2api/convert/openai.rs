...
pub fn tools_section(tools: &[Value], tool_choice: &ToolChoice) -> Option<String> {
    ...
    Some(format!(
        "[System instruction]: {}",
        tool_use_section(BlockKind::ToolCall, INSTRUCTION_OPENAI, &defs, &tool_choice.instruction())
    ))
}
            ...
            for part in parts {
                match part.get("type").and_then(Value::as_str) {
                    Some("text") | Some("input_text") => {
                        if let Some(t) = part.get("text").and_then(Value::as_str) { out.push(t.to_string()); }
                    }
                    Some("image_url") => out.push("[Note: Image input not supported by this server; describe the image in text]. ".to_string()),
                    _ => {}
                }
            }
            out.join(" ")
...
pub fn messages_to_prompt(messages: &[Value], tools: Option<&[Value]>, tool_choice: &ToolChoice) -> String {
    let mut parts = Vec::new();
    if let Some(tools) = tools {
        if !tool_choice.is_none() {
            if let Some(section) = tools_section(tools, tool_choice) { parts.push(section); }
        }
    }
    for msg in messages {
        ...
        match role {
            ...
            "assistant" => match msg.get("tool_calls") {
                Some(Value::Array(tcs)) if !tcs.is_empty() => {
                    ...
                    for tc in tcs {
                        ...
                        let args: Value = f.get("arguments")
                            .and_then(|a| serde_json::from_str(a.as_str()?).ok())
                            .unwrap_or_else(|| json!({}));
                        blocks.push(tool_call_block(name, &args));
                    }
                    let prefix = if content.is_empty() { String::new() } else { format!("{content}\n") };
            ...
            _ => if !content.is_empty() { parts.push(content); },
        }
    }
    parts.join("\n\n")
}
...
mod tests {
    ...
    fn tool_choice_none_occulte_la_section_tools() {
        let tools = vec![json!({"type":"function","function":{"name":"lire","description":"d","parameters":{}}})];
