...
pub fn resolve_model_strict(
    requested: &str,
    default: &str,
) -> Result<crate::core::models::Resolved, String> {
    let base = requested.split("@think=").next().unwrap_or(requested);
    if !crate::core::models::MODEL_KEYS.contains(&base) {
...
mod tests {
    ...
    fn tool_choice_parse_et_instruction() {
        ...
        assert_eq!(ToolChoice::parse(Some(&json!("required"))), ToolChoice::Required);
        assert_eq!(ToolChoice::parse(Some(&json!("auto"))), ToolChoice::Auto);
        assert_eq!(ToolChoice::parse(Some(&json!({"type":"function","function":{"name":"lire"}}))), ToolChoice::Named("lire".into()));
        assert!(ToolChoice::Auto.instruction().is_empty());
        assert!(ToolChoice::Required.instruction().contains("at least one tool"));
        assert!(ToolChoice::Named("lire".into()).instruction().contains("lire"));
