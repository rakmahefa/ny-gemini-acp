...
impl GeminiFrameDecoder {
    ...
    fn decode_line(&mut self, line: &str) -> Vec<GeminiFrameEvent> {
        ...
        let Ok(value) = serde_json::from_str::<Value>(trimmed) else {
            tracing::trace!(
                bytes = trimmed.len(),
                "GeminiFrameDecoder ignored non-JSON line"
            );
            return Vec::new();
        };
    ...
    fn decode_value(&mut self, value: Value) -> Vec<GeminiFrameEvent> {
        ...
        let Some(record) = first.as_array() else {
            return Vec::new();
        };
        if record.first().and_then(Value::as_str) != Some("wrb.fr") {
            return Vec::new();
        }
        let Some(inner) = record.get(2) else {
            return Vec::new();
        };
        match inner {
            Value::String(raw) => match serde_json::from_str::<Value>(raw) {
                Ok(inner_value) => self.decode_inner(inner_value),
                Err(_) => vec![GeminiFrameEvent::Metadata {
                    kind: "unparsed_frame".into(),
                    value: bounded_json(raw),
                }],
            },
            Value::Object(_) => self.decode_inner(inner.clone()),
            other => vec![GeminiFrameEvent::Metadata {
                kind: "unexpected_inner".into(),
                value: bounded_value(other.clone()),
            }],
        }
    }
    ...
    fn decode_inner(&mut self, inner: Value) -> Vec<GeminiFrameEvent> {
        ...
        if let Some(candidates) = inner.get(4).and_then(Value::as_array) {
            ...
            if let Some(text) = longest {
                events.push(GeminiFrameEvent::Text(text));
            }
        }
        events
    }
...
fn collect_tool_calls(
    inner: &Value,
    events: &mut Vec<GeminiFrameEvent>,
            ...
            Value::Object(map) => {
                for key in [
                    ...
                    if let Some(call) = map.get(key).and_then(parse_tool_object) {
                        ...
                        if seen.len() >= MAX_TOOL_EVENTS_PER_STREAM || !seen.insert(id.clone()) {
                            continue;
                        }
                ...
                for value in map.values() {
                    walk(value, events, seen, next_id);
                }
            }
            Value::Array(values) => {
                for value in values {
...
fn collect_metadata(inner: &Value, events: &mut Vec<GeminiFrameEvent>) {
    ...
    for key in ["usageMetadata", "usage", "finishReason", "blockReason"] {
        if let Some(value) = map.get(key) {
            ...
            {
                events.push(GeminiFrameEvent::Metadata {
                    kind: key.to_owned(),
                    value: value.clone(),
                });
            } else {
...
fn bounded_value(value: Value) -> Value {
    serde_json::json!({"preview": value.to_string().chars().take(512).collect::<String>()})
}
fn bounded_json(raw: &str) -> Value {
    serde_json::json!({"bytes": raw.len(), "preview": raw.chars().take(512).collect::<String>()})
}
...
pub fn is_empty_stream(raw: &str) -> bool {
    ...
    decoder.feed(raw).into_iter().all(|event| {
        !matches!(event, GeminiFrameEvent::Text(ref text) if !text.is_empty())
...
mod tests {
    ...
    fn structured_tool_call_is_preserved() {
        let inner =
            json!({"functionCall": {"id": "c1", "name": "glob", "arguments": {"pattern": "*.rs"}}});
    ...
    fn duplicate_structured_tool_call_is_emitted_once() {
        ...
        assert_eq!(
            first
                .iter()
                .filter(|event| matches!(event, GeminiFrameEvent::ToolCall { .. }))
                .count(),
            1
        );
        assert_eq!(
    ...
    fn longest_candidate_is_selected() {
        ...
        assert!(events.iter().any(
            |event| matches!(event, GeminiFrameEvent::Text(text) if text == "Bonjour, le monde")
        ));
    }
    #[test]
    fn final_partial_line_is_flushed() {
        ...
        assert!(events
            .iter()
            .any(|event| matches!(event, GeminiFrameEvent::Text(text) if text == "abc")));
    }
    #[test]
