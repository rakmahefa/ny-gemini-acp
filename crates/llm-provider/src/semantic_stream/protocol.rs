...
use super::parsers::{
    allocate_call_id, parse_bare_json, parse_follow_up_candidates, parse_inline_tool_call,
};
use super::types::{
...
impl ProtocolDetector {
    ...
    fn drain(&mut self, final_flush: bool) -> Vec<ProtocolEvent> {
        ...
        loop {
            let progressed = match self.mode {
                ...
                ProtocolMode::InlineToolCall => {
                    self.drain_inline_tool_call(&mut events, final_flush)
                }
            };
            if !progressed {
                break;
    ...
    fn drain_normal(&mut self, events: &mut Vec<ProtocolEvent>, final_flush: bool) -> bool {
        if let Some((start, kind)) = self.find_result_marker() {
            ...
            let closing =
                if first_line.contains("```tool_call") || first_line.contains("```function_call") {
                    Some("```")
    ...
    fn find_result_marker(&self) -> Option<(usize, ResultMarker)> {
        [
            self.pending
                ...
                .map(|i| (i, ResultMarker::Legacy)),
            self.pending
                .find(TOOL_RESULT_ENVELOPE)
                .map(|i| (i, ResultMarker::Legacy)),
            self.pending
                .find(TOOL_RESULT_INLINE)
                .map(|i| (i, ResultMarker::Inline)),
        ]
        .into_iter()
        .flatten()
        .min_by_key(|(i, _)| *i)
    ...
    fn find_partial_tool_marker(&self) -> Option<usize> {
        [
            BlockKind::Tool.opening(),
            BlockKind::SingleQuoteTool.opening(),
            BlockKind::Function.opening(),
        ]
        .into_iter()
    ...
    fn drain_inline_tool_call(
        ...
        self.pending.drain(..newline + 1);
        if let Some(call) =
            parse_inline_tool_call(&format!("[tool_call {line}"), &mut self.next_call_id)
        {
            events.push(ProtocolEvent::ToolCall(call));
        }
        self.mode = ProtocolMode::Normal;
