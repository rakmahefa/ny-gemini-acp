...
use super::parsers::{allocate_call_id, parse_bare_json, parse_follow_up_candidates, parse_inline_tool_call};
use super::types::{
    BlockKind, ModelToolCall, ProtocolEvent, ProtocolMode, FOLLOW_UP_PREFIX, MAX_FOLLOW_UP,
    MAX_PENDING, MAX_TOOL_BLOCK, TOOL_CALL_INLINE, TOOL_RESULT_ENVELOPE, TOOL_RESULT_INLINE,
    TOOL_RESULT_PREFIX,
};
...
impl ProtocolDetector {
    ...
    fn drain_normal(&mut self, events: &mut Vec<ProtocolEvent>, final_flush: bool) -> bool {
        if let Some((start, kind)) = self.find_result_marker() {
            if start > 0 {
            ...
            if kind == ResultMarker::Inline {
                self.pending.drain(..TOOL_RESULT_INLINE.len());
                self.mode = ProtocolMode::IgnoreInlineToolResult;
                self.at_stream_start = false;
                return true;
            }
            let Some(newline) = self.pending.find('\n') else {
                if final_flush { self.pending.clear(); }
                return false;
            };
            let first_line = self.pending[..newline].trim_end_matches(['\r', '\n']);
            let closing = if first_line.contains("```tool_call") || first_line.contains("```function_call") { Some("```") } else if first_line.contains("'''tool_call") { Some("'''") } else { None };
        ...
        if let Some(start) = self.pending.find(TOOL_CALL_INLINE) {
            if start > 0 {
                let text = self.pending[..start].to_owned();
                self.pending.drain(..start);
                self.emit_text(&text, events);
        ...
        if let Some(start) = partial_marker_suffix(&self.pending, TOOL_CALL_INLINE) {
            if start > 0 {
                let text = self.pending[..start].to_owned();
                self.pending.drain(..start);
                self.emit_text(&text, events);
    ...
    fn find_result_marker(&self) -> Option<(usize, ResultMarker)> {
        [
            self.pending.find(TOOL_RESULT_PREFIX).map(|i| (i, ResultMarker::Legacy)),
            self.pending.find(TOOL_RESULT_ENVELOPE).map(|i| (i, ResultMarker::Legacy)),
            self.pending.find(TOOL_RESULT_INLINE).map(|i| (i, ResultMarker::Inline)),
        ].into_iter().flatten().min_by_key(|(i, _)| *i)
    }
    ...
    fn drain_inline_tool_result(&mut self, final_flush: bool) -> bool {
    ...
    fn drain_inline_tool_call(&mut self, events: &mut Vec<ProtocolEvent>, final_flush: bool) -> bool {
        let Some(newline) = self.pending.find('\n') else {
            if final_flush { self.pending.clear(); self.mode = ProtocolMode::Normal; }
            return false;
        };
        ...
        self.pending.drain(..newline + 1);
        if let Some(call) = parse_inline_tool_call(&format!("[tool_call {line}"), &mut self.next_call_id) {
            events.push(ProtocolEvent::ToolCall(call));
        }
        self.mode = ProtocolMode::Normal;
        true
        ...
        let Some(end) = self.pending.find(closing) else {
            if let ProtocolMode::ToolBlock { body, oversized, .. } = &mut self.mode {
                if !*oversized {
                    ...
                    self.pending.clear();
                    if body.len() > MAX_TOOL_BLOCK { *oversized = true; body.clear(); }
    ...
    fn emit_text(&self, text: &str, events: &mut Vec<ProtocolEvent>) {
        if !text.is_empty() { events.push(ProtocolEvent::Text(normalize_assistant_marker(text))); }
    }

    fn emit_tool_block(&mut self, _kind: BlockKind, body: &str, events: &mut Vec<ProtocolEvent>) {
        let Ok(value) = serde_json::from_str::<serde_json::Value>(body.trim()) else { return; };
        let Some(name) = value.get("name").and_then(serde_json::Value::as_str) else { return; };
        let id = value.get("id").or_else(|| value.get("call_id")).and_then(serde_json::Value::as_str).filter(|id| !id.trim().is_empty()).map(ToOwned::to_owned).unwrap_or_else(|| allocate_call_id(&mut self.next_call_id));
    ...
    fn emit_follow_ups(&mut self, text: &str, events: &mut Vec<ProtocolEvent>) {
        if let Some(calls) = parse_follow_up_candidates(text) {
            for (label, query) in calls {
                events.push(ProtocolEvent::ToolCall(ModelToolCall { id: self.allocate_id(), name: "FollowUp".into(), arguments: json!({ "label": label, "query": query }) }));
            }
        }
    }

    fn allocate_id(&mut self) -> String { allocate_call_id(&mut self.next_call_id) }
}
...
fn partial_marker_suffix(input: &str, marker: &str) -> Option<usize> {
    ...
    for len in (1..=max).rev() {
        ...
        if !input.is_char_boundary(start) { continue; }
        if marker.starts_with(&input[start..]) { return Some(start); }
    }
    None
}
...
fn normalize_assistant_marker(text: &str) -> String {
    ...
    if let Some(rest) = trimmed.strip_prefix("[Assistant]:") { rest.trim_start().to_owned() } else { text.to_owned() }
}
