...
impl LlmProvider for GeminiProvider {
    async fn stream(&self, request: LlmRequest) -> Result<LlmStream, String> {
        self.client.stream(&request.prompt, &request.model, request.think, &request.refs)
            .await.map_err(|error| format!("{error:#}"))
    }

    async fn upload_image(&self, base64: &str, mime: &str) -> Result<String, String> {
        self.client.upload_image(base64, mime).await.map_err(|error| format!("{error:#}"))
    }

    fn model_info(&self, model: &str) -> LlmModelInfo {
        let supports_thinking = crate::core::models::resolve(model, crate::core::models::DEFAULT_MODEL)
