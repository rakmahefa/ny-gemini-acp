...
impl GeminiSemanticStream {
    ...
    fn feed_detecting(&mut self, delta: &str) -> Vec<ModelEvent> {
        ...
        if let Some(marker) = matching_marker(&self.pending, &REASONING_OPEN_MARKERS) {
            self.pending.drain(..marker.len());
            self.phase = ReasoningPhase::Reasoning;
            let mut events = Vec::new();
            events.append(&mut self.feed_reasoning(""));
            return events;
        }
...
fn matching_marker(buffer: &str, markers: &[&str]) -> Option<&'static str> {
    markers.iter().copied().find(|marker| buffer.starts_with(marker))
}
...
mod tests {
    ...
    fn non_reasoning_models_pass_text_through_unchanged() {
        ...
        assert_eq!(
            events(&mut stream, &["<thinking>hidden</thinking>answer"]),
    ...
    fn reasoning_envelope_becomes_semantic_events() {
        ...
        assert_eq!(
            events(
                &mut stream,
                &["<thinking>raisonnement", " utile</thinking>", "réponse"],
            ),
            vec![
    ...
    fn reasoning_open_marker_split_across_chunks_is_atomic() {
        ...
        assert_eq!(
            stream.feed("nking>pensée</thinking>réponse"),
            vec![
    ...
    fn reasoning_close_marker_split_across_chunks_is_atomic() {
        let mut stream = GeminiSemanticStream::new(true);
        assert_eq!(
            stream.feed("<thinking>pensée utile </thi"),
            vec![ModelEvent::ReasoningDelta("pensée utile ".into())]
        );
        assert_eq!(
    ...
    fn non_enveloped_reasoning_model_response_stays_text() {
        let mut stream = GeminiSemanticStream::new(true);
        assert_eq!(
            events(&mut stream, &["réponse finale", " avec détails"]),
            vec![
                ModelEvent::TextDelta("réponse finale".into()),
                ModelEvent::TextDelta(" avec détails".into()),
            ]
        );
    }
}