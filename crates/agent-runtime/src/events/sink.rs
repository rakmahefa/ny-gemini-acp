...
pub trait TurnEventSink: Send {
    ...
    fn turn_cancelled(&mut self) -> bool { self.turn_terminated(TurnTermination::Cancelled) }
    fn turn_failed(&mut self) -> bool { self.turn_terminated(TurnTermination::Failed) }
...
impl TurnEventSink for TurnEventEmitter {
    ...
    fn ensure_turn_started(&mut self) -> bool { TurnEventEmitter::ensure_turn_started(self) }
    ...
    fn tool_call_requested(&mut self, upstream_id: String, name: String, ui: Option<ToolUiModel>) -> bool { TurnEventEmitter::tool_call_requested_with_ui(self, upstream_id, name, ui) }
    fn permission_requested(&mut self, upstream_id: String) -> bool { TurnEventEmitter::permission_requested(self, upstream_id) }
    fn tool_execution_started(&mut self, upstream_id: String, ui: Option<ToolUiModel>) -> bool { TurnEventEmitter::tool_execution_started_with_ui(self, upstream_id, ui) }
    fn tool_result_received(&mut self, upstream_id: String, result: String, ui: Option<ToolUiModel>) -> bool { TurnEventEmitter::tool_result_received_with_ui(self, upstream_id, result, ui) }
    fn turn_terminated(&mut self, termination: TurnTermination) -> bool {
        match termination {
