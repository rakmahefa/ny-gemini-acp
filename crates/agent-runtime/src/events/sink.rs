...
///
/// Terminal failure/cancellation share one semantic operation. The compatibility helpers
/// remain as defaults so existing callers keep the same surface while all termination
/// policy is centralized in `turn_terminated`.
