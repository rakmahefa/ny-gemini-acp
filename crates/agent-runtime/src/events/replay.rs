...
impl SemanticJournal {
    ...
    pub fn events(&self) -> &[SemanticEvent] { &self.events }
    pub fn into_events(self) -> Vec<SemanticEvent> { self.events }

    pub fn audit(&self) -> Result<ReplayDiagnostic, String> {
        let Some(first) = self.events.first() else { return Err("semantic journal is empty".into()); };
        ...
        for event in &self.events {
            ...
            if event_session != session_id || event_turn != turn_id {
                return Err("semantic journal mixes session/turn identities".into());
            }
            let sequence = event_sequence(event);
            if sequence != expected {
                return Err(format!("semantic journal sequence gap: expected {expected}, got {sequence}"));
            }
            expected = expected.checked_add(1).ok_or_else(|| "semantic journal sequence overflow".to_owned())?;
            if let Some(kind) = terminal_kind(event) {
                if terminal_event.is_some() { return Err("semantic journal contains multiple terminal events".into()); }
                terminal_event = Some(kind.to_owned());
            } else if terminal_event.is_some() {
                return Err("semantic journal contains events after terminality".into());
            }
        }
        Ok(ReplayDiagnostic {
    ...
    pub fn to_json_lines(&self) -> Result<String, serde_json::Error> {
        self.events.iter().map(serde_json::to_string).collect::<Result<Vec<_>, _>>().map(|lines| lines.join("\n"))
    }
    ...
    pub fn from_json_lines(input: &str) -> Result<Self, String> {
        ...
        for (line_number, line) in input.lines().enumerate() {
            ...
            let event: SemanticEvent = serde_json::from_str(line)
                .map_err(|error| format!("invalid semantic journal line {}: {error}", line_number + 1))?;
            journal.push(event)?;
        }
        Ok(journal)
    }
    ...
    fn audit_candidate(&self, event: &SemanticEvent) -> Result<(), String> {
        ...
        if let Some(last) = self.events.last() {
            ...
            if last_session != session_id || last_turn != turn_id { return Err("semantic journal mixes session/turn identities".into()); }
            let expected = event_sequence(last).checked_add(1).ok_or_else(|| "semantic journal sequence overflow".to_owned())?;
            if event_sequence(event) != expected { return Err(format!("semantic journal sequence gap: expected {expected}, got {}", event_sequence(event))); }
            if terminal_kind(last).is_some() { return Err("semantic journal is already terminal".into()); }
        } else if event_sequence(event) != 0 {
            return Err(format!("semantic journal must start at sequence 0, got {}", event_sequence(event)));
        }
        Ok(())
    }
}
...
fn event_sequence(event: &SemanticEvent) -> u64 { event_context(event).sequence }
...
mod tests {
    ...
    fn started(sequence: u64) -> SemanticEvent {
        SemanticEvent::TurnStarted { context: EventContext::new("session", "turn", sequence) }
    }
    ...
    fn journal_round_trips_and_audits_deterministically() {
        ...
        journal.push(SemanticEvent::TurnCompleted { context: EventContext::new("session", "turn", 1) }).unwrap();
        let encoded = journal.to_json_lines().unwrap();
        assert_eq!(SemanticJournal::from_json_lines(&encoded).unwrap(), journal);
        assert_eq!(journal.audit().unwrap().terminal_event.as_deref(), Some("turn_completed"));
    }

    #[test]
    fn audit_rejects_sequence_gap() {
        let journal = SemanticJournal { events: vec![started(0), started(2)] };
        assert!(journal.audit().unwrap_err().contains("sequence gap"));
    }

    #[test]
    fn audit_rejects_events_after_terminality() {
        let journal = SemanticJournal { events: vec![
            started(0),
            SemanticEvent::TurnCompleted { context: EventContext::new("session", "turn", 1) },
            started(2),
        ]};
