...
impl TurnEventEmitter {
    ...
    fn publish(&self, event: SemanticEvent) -> bool {
        if self.require_transport {
            match self.bus.publish_turn(event.clone()) {
                ...
                Err(error) => {
                    tracing::error!(
                        ...
                        turn = %self.turn_id,
                        error = %error,
                        "mandatory ACP semantic transport failed"
                    );
                    false
                }
            }
            ...
            if self.bus.has_turn_subscriber(self.turn_id.as_str()) {
                let _ = self.bus.publish_turn(event);
            }
            true
        }
    }

    fn bind_tool_identity(
        &mut self,
        upstream_id: ToolCallId,
    ) -> Result<ToolCallId, IntegrityError> {
    ...
    pub fn turn_started(&mut self) -> bool {
        ...
        if let Err(error) = candidate.turn_started() {
    ...
    pub fn assistant_started(&mut self) -> bool {
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.assistant_started() {
            return self.reject(error);
        }

        let event = SemanticEvent::AssistantStarted {
            context: self.proposed_context(),
        };
        if self.publish(event) {
            self.integrity = candidate;
    ...
    pub fn assistant_delta(&mut self, delta: impl Into<String>) -> bool {
        ...
        if let Err(error) = self.integrity.assistant_delta() {
            return self.reject(error);
        }

        let event = SemanticEvent::AssistantDelta {
            context: self.proposed_context(),
            delta: delta.into(),
        };
        if self.publish(event) {
    ...
    pub fn assistant_completed(&mut self) -> bool {
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.assistant_completed() {
            return self.reject(error);
        }

        let event = SemanticEvent::AssistantCompleted {
            context: self.proposed_context(),
        };
        if self.publish(event) {
            self.integrity = candidate;
    ...
    pub fn assistant_yields_to_action(&mut self) -> bool {
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.assistant_yields_to_action() {
            return self.reject(error);
        }
        self.integrity = candidate;
        true
    ...
    pub fn thinking_started(&mut self) -> bool {
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.thinking_started() {
            return self.reject(error);
        }

        let event = SemanticEvent::ThinkingStarted {
            context: self.proposed_context(),
        };
        if self.publish(event) {
            self.integrity = candidate;
    ...
    pub fn thinking_delta(&mut self, delta: impl Into<String>) -> bool {
        ...
        if let Err(error) = self.integrity.thinking_delta() {
            return self.reject(error);
        }

        let event = SemanticEvent::ThinkingDelta {
            context: self.proposed_context(),
            delta: delta.into(),
        };
        if self.publish(event) {
    ...
    pub fn thinking_completed(&mut self) -> bool {
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.thinking_completed() {
            return self.reject(error);
        }

        let event = SemanticEvent::ThinkingCompleted {
            context: self.proposed_context(),
        };
        if self.publish(event) {
            self.integrity = candidate;
    ...
    pub fn tool_call_requested_with_ui(
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.tool_call_requested(semantic_id.as_str()) {
            self.rollback_tool_binding(&upstream_id);
            return self.reject(error);
        }

        let event = SemanticEvent::ToolCallRequested {
            context: ToolEventContext {
                event: self.proposed_context(),
                tool_call_id: semantic_id,
            },
            name: name.into(),
            ui,
        };
    ...
    pub fn permission_requested(&mut self, upstream_id: impl Into<ToolCallId>) -> bool {
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.permission_requested(semantic_id.as_str()) {
            return self.reject(error);
        }

        let event = SemanticEvent::PermissionRequested {
            context: ToolEventContext {
                event: self.proposed_context(),
                tool_call_id: semantic_id,
            },
    ...
    pub fn tool_execution_started_with_ui(
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.tool_execution_started(semantic_id.as_str()) {
            return self.reject(error);
        }
    ...
    pub fn tool_result_received_with_ui(
        ...
        let mut candidate = self.integrity.clone();
        if let Err(error) = candidate.tool_result_received(semantic_id.as_str()) {
            return self.reject(error);
        }

        let event = SemanticEvent::ToolResultReceived {
            context: ToolEventContext {
                event: self.proposed_context(),
                tool_call_id: semantic_id,
            },
    ...
    fn finish_terminal(&mut self, event: &str) -> bool {
        ...
        if event != "turn_completed" {
            ...
            if candidate.thinking_active() {
                if let Err(error) = candidate.thinking_completed() {
                    return self.reject(error);
                }
            }
            if candidate.assistant_active() {
                if let Err(error) = candidate.assistant_completed() {
                    return self.reject(error);
                }
            }
            if let Err(error) = candidate.abort_open_tools(reason) {
                return self.reject(error);
            }
        }

        if let Err(error) = candidate.finish_terminal_after_scopes(event) {
            return self.reject(error);
        }
