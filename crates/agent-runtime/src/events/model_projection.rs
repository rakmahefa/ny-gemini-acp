        ...
        let item = tokio::select! {
            ...
            changed = cancel_rx.changed() => {
                if changed.is_ok() && *cancel_rx.borrow() {
                    close_active_scopes_best_effort(sink, &mut thinking_active, &mut assistant_active);
...
mod tests {
    ...
    async fn projects_reasoning_then_text_without_agent_loop() {
        ...
        tx.send(Ok(ModelEvent::ReasoningDelta("think".into())))
            .await
            .unwrap();
        tx.send(Ok(ModelEvent::TextDelta("answer".into())))
            .await
            .unwrap();
        drop(tx);
    ...
    async fn rejects_reasoning_after_text() {
        ...
        tx.send(Ok(ModelEvent::TextDelta("answer".into())))
            .await
            .unwrap();
        tx.send(Ok(ModelEvent::ReasoningDelta("late".into())))
            .await
            .unwrap();
        drop(tx);
        let mut harness = emitter();
        let error = consume_model_stream(rx, &Cancellation::new(), &mut harness.emitter)
            .await
            .unwrap_err();
        assert!(
            matches!(error, ModelProjectionError::InvalidSequence(message) if message.contains("reasoning resumed"))
        );
    }
    ...
    async fn closes_scopes_before_returning_provider_error() {
        ...
        tx.send(Ok(ModelEvent::ReasoningDelta("think".into())))
            .await
            .unwrap();
        tx.send(Err(LlmError::Provider("boom".into())))
            .await
            .unwrap();
        drop(tx);
        let mut harness = emitter();
