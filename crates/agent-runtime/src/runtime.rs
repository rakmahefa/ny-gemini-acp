...
#[derive(Debug, Clone)]
pub struct RuntimeConfig { pub data_dir: PathBuf, pub default_model: String }
#[derive(Clone)]
pub struct AppState {
...
impl AgentRuntime {
    pub async fn new(config: RuntimeConfig, llm: SharedLlmProvider, tools: SharedToolProvider) -> Result<Self> {
        ...
        Ok(Self { state: AppState { store, sessions, turns: TurnManager::new(), llm, tools, config: Arc::new(config), events: EventBus::new() } })
    }
    ...
    pub async fn shutdown(&self) {
        match tokio::time::timeout(SHUTDOWN_TIMEOUT, self.state.turns.cancel_all_and_wait()).await {
            Ok(Ok(count)) => tracing::info!(cancelled_turns = count, "arrêt des tours actifs terminé"),
