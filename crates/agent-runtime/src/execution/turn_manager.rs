...
impl TurnManager {
    ...
    /// This is the required synchronization barrier before session close/delete.
...
mod tests {
    ...
    async fn concurrent_starts_have_exactly_one_winner() {
        ...
        let first = tokio::spawn(async move { m1.start("session", move |cancellation| async move {
            s1.fetch_add(1, Ordering::SeqCst); let mut rx = cancellation.subscribe();
            tokio::select! { _ = b1.notified() => Ok(()), _ = rx.changed() => Ok(()) }
        }).await });
        let second = tokio::spawn(async move { m2.start("session", move |cancellation| async move {
            s2.fetch_add(1, Ordering::SeqCst); let mut rx = cancellation.subscribe();
    ...
    async fn cancel_and_wait_removes_turn_before_return() {
        let manager = TurnManager::new();
        let handle = manager.start("session", |cancellation| async move {
    ...
    async fn cancel_all_signals_every_active_turn() {
        ...
        wait_for_terminal(&a).await; wait_for_terminal(&b).await;
        assert_eq!(a.state().await, TurnState::Cancelled); assert_eq!(b.state().await, TurnState::Cancelled);
    }
}
