...
impl AgentLoop {
        ...
        F: Fn(&Session, &dyn ToolProvider) -> String,
    {
        ...
        for round in 0..self.config.max_rounds {
            ...
            let stream = self.llm.stream(request).await.map_err(|error| {
        ...
        let item = tokio::select! {
            ...
            changed = cancel_rx.changed() => {
                if changed.is_ok() && *cancel_rx.borrow() {
                    if thinking_active { let _ = emitter.thinking_completed(); }
                    if assistant_active { let _ = emitter.assistant_completed(); }
                    let _ = emitter.turn_cancelled();
                    return Err(AgentLoopError::Cancelled);
                }
                continue;
...
mod tests {
    ...
    #[tokio::test]
    async fn rejects_empty_stream_as_no_progress() {
        let llm = Arc::new(FakeLlm::default());
        llm.rounds.lock().unwrap().push_back(vec![]);
        let loop_ = AgentLoop::new(llm, Arc::new(FakeTools), AgentLoopConfig::default()).unwrap();
        let mut session = session();
    ...
    #[tokio::test]
    async fn reasoning_is_closed_before_text() {
        let llm = Arc::new(FakeLlm::default());
        llm.rounds.lock().unwrap().push_back(vec![
