...
impl AgentLoopConfig {
    fn validate(self) -> Result<Self, AgentLoopError> {
        ...
        if self.max_tool_calls_per_round == 0 { return Err(AgentLoopError::InvalidConfig("max_tool_calls_per_round must be greater than zero".into())); }
        Ok(self)
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AgentLoopOutcome { pub output: String, pub rounds: usize, pub tool_calls: usize }

#[derive(Debug, thiserror::Error)]
pub enum AgentLoopError {
    #[error("invalid agent loop configuration: {0}")] InvalidConfig(String),
    #[error("agent loop cancelled")] Cancelled,
    #[error("invalid session: {0}")] InvalidSession(String),
    #[error("LLM provider failed: {0}")] Llm(#[source] LlmError),
    #[error("LLM stream produced no observable events")] EmptyStream,
    #[error("LLM stream produced no final text or tool call")] NoProgress,
...
fn validate_session(session: &Session) -> Result<(), AgentLoopError> {
    if session.id.trim().is_empty() { return Err(AgentLoopError::InvalidSession("session id must not be empty".into())); }
...
async fn consume_stream(stream: crate::LlmStream, cancellation: &Cancellation, emitter: &mut TurnEventEmitter) -> Result<ModelRound, AgentLoopError> {
    match consume_model_stream(stream, cancellation, emitter).await {
        Ok(round) => Ok(round),
        Err(ModelProjectionError::Cancelled) => { let _ = emitter.turn_cancelled(); Err(AgentLoopError::Cancelled) }
        Err(ModelProjectionError::Llm(error)) => { let _ = emitter.turn_failed(); Err(AgentLoopError::Llm(error)) }
        Err(ModelProjectionError::InvalidSequence(message)) => { let _ = emitter.turn_failed(); Err(AgentLoopError::InvalidToolCall(message)) }
...
mod tests {
    ...
    #[derive(Default)] struct FakeLlm { rounds: Mutex<VecDeque<Vec<Result<ModelEvent, LlmError>>>> }
    #[async_trait::async_trait]
