...
`agent-runtime` owns semantic lifecycle, tool identity and the canonical `ToolUiModel` presentation contract. `tools-provider/tool_ux` is the rich semantic builder used to create that presentation model.

`tool_ux` MUST remain host-neutral. It MUST NOT construct ACP presentation types. ACP-specific `ToolKind`, `ToolCallContent`, `ToolCallLocation`, `ToolCallStatus`, `ToolCall`, and `ToolCallUpdate` values are created only by `acp-adaptor`.
...
Any future tool UX feature MUST update the semantic `ToolUiModel` contract first. A new direct `tools-provider → ACP` visual path is prohibited. The visual pipeline must remain single-path end to end.
